...
foreach ($module in $modules) {
    $modPath = Join-Path $AzureDir $module
    if (Test-Path $modPath) {
        Write-Host ""
        Write-Host ">>> $module" -ForegroundColor Magenta
        & $modPath
    } else {
...
$naCount = ($Global:CISAuditResults.Values | Where-Object { $_.Status -eq "N/A" }).Count
$evaluatedCount = $tot - $naCount
$complianceRate = if ($evaluatedCount -gt 0) { [math]::Round($complianceCount / $evaluatedCount * 100, 1) } else { 0 }

Write-Host ""
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host "  RIEPILOGO: $complianceCount/$evaluatedCount COMPLIANT ($complianceRate`%)  [$naCount N/A esclusi, $tot check totali]" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
